...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if old_reason:
            message.reply_text("Orang ini sudah ter gban.\nAlasannya:"
                               "<code>{}</code>\n"
                               "Gbanned berhasil!".format(html.escape(old_reason)),
                               parse_mode=ParseMode.HTML)
        else:
            message.reply_text("Orang ini sudah ter gban, tapi tidak ada alasan yang ditetapkan; I've gone and updated it!")
    ...
    text12 = f"*⚡️Another Bitch Goes Off⚡️* {ok123}."
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                message.reply_text("Tidak bisa gban karena: {}".format(excp.message))
                send_to_list(bot, SUDO_USERS + SUPPORT_USERS, "Tidak bisa gban karena: {}".format(excp.message))
    ...
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS,
                   "{} GBAN TELAH BERHASIL!".format(mention_html(user_chat.id, user_chat.first_name)),
                   html=True)
...
def ungban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text("Kasian ah {} di gban bhahaha".format(user_chat.first_name))
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in UNGBAN_ERRORS:
                ...
                message.reply_text("Tidak bisa un-gban karena: {}".format(excp.message))
                bot.send_message(OWNER_ID, "Tidak bisa un-gban karena: {}".format(excp.message))
                return
        except TelegramError:
            pass
    ...
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS,
                  "{} berhasil un-gbanned!".format(mention_html(user_chat.id,
                                                                         user_chat.first_name)),
                 html=True)

    message.reply_text("Seseorang sudah ter un-gbanned.The hardest choices require the strongest wills.")
...
def gmute(bot: Bot, update: Update, args: List[str]):
    ...
    if sql.is_user_gmuted(user_id):
        if not reason:
            message.reply_text("Orang ini sudah ter gmuted hmm...")
            return

        success = sql.update_gmute_reason(user_id, user_chat.username or user_chat.first_name, reason)
        if success:
            message.reply_text("Orang ini sudah ter gmute!")
        else:
    ...
    message.reply_text("*Bismillah gmute HAHA*")

    muter = update.effective_user  # type: Optional[User]
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS,
                 "{} sedang gmute {} "
                 "\nKarena: {}".format(mention_html(muter.id, muter.first_name),
                                       mention_html(user_chat.id, user_chat.first_name), reason or "Tidak ada alasan"),
                 html=True)
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message == "Dia adalah admin":
                pass
                ...
                message.reply_text("Tidak bisa gmute karena: {}".format(excp.message))
                send_to_list(bot, SUDO_USERS + SUPPORT_USERS, "Tidak bisa gmute karena: {}".format(excp.message))
    ...
    message.reply_text("Dia telah ter gmute.")
