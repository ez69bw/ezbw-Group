...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text(tld(chat.id, "Tag orangnya."))
        return ""
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text(tld(chat.id, "Saya tidak bisa menemukan orang ini"))
            return ""
        else:
            raise
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(tld(chat.id, "Tidak bisa ban admin."))
        return ""
    ...
    reply = "{} Telah di ban HAHA!".format(mention_html(member.user.id, member.user.first_name))
    ...
    try:
        ...
        if excp.message == "Reply tidak ditemukan":
            # Do not reply
            #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
            ...
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text(tld(chat.id, "Well damn, saya tidak bisa ban dia."))
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("Saya tidak bisa menemukan orang ini")
            return ""
        else:
            raise

    if user_id == bot.id:
        message.reply_text("Anda gabut? Saya tidak bisa kick diri sendiri!")
        return ""

    if is_user_ban_protected(chat, user_id):
        message.reply_text("Kenapa mau kick admin? That sounds like a pretty dumb idea.")
        return ""
    ...
    if res:
        ...
        message.reply_text("Well damn, Saya tidak bisa kick dia.")
...
def kickme(bot: Bot, update: Update):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("Anda gabut? atau gila?")
        return

    res = update.effective_chat.unban_member(user_id)  # unban on current user = kick
    if res:
        update.effective_message.reply_text("bye kintil.")
    else:
