...
def unmute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member.status != 'kicked' and member.status != 'left':
        if member.can_send_messages and member.can_send_media_messages \
            ...
            message.reply_text(tld(chat.id, "Orang ini sudah bisa nimbrung di {}.").format(chatD.title))
        else:
            ...
            keyboard = []
            reply = tld(chat.id, "hai, {} nimbrung dong sini di {} mwehehe!").format(mention_html(member.user.id, member.user.first_name), chatD.title)
            message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
            return "<b>{}:</b>" \
                   "\n#UNMUTE" \
                   "\n<b>• Admin:</b> {}" \
...
def nomedia(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member:
        if is_user_admin(chatD, user_id, member=member):
            ...
            keyboard = []
            reply = tld(chat.id, "{} Tidak bisa mengirim media di {}!").format(mention_html(member.user.id, member.user.first_name), chatD.title)
            message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
            return "<b>{}:</b>" \
                   "\n#RESTRICTED" \
                   "\n<b>• Admin:</b> {}" \
            ...
            message.reply_text(tld(chat.id, "Orang ini sudah dibatasi di {}!"))
    else:
        message.reply_text(tld(chat.id, "Dia tidak ada di {}!").format(chatD.title))
...
def media(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member.status != 'kicked' and member.status != 'left':
        if member.can_send_messages and member.can_send_media_messages \
                and member.can_send_other_messages and member.can_add_web_page_previews:
            message.reply_text(tld(chat.id, "Orang ini sudah dibebaskan di {}.").format(chatD.title))
        else:
            bot.restrict_chat_member(chatD.id, int(user_id),
                                     can_send_messages=True,
                                     can_send_media_messages=True,
                                     ...
                                     can_add_web_page_previews=True)
            keyboard = []
            reply = tld(chat.id, "Okey, {} sudah bisa mengirim media di {}!").format(mention_html(member.user.id, member.user.first_name), chatD.title)
            message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
            return "<b>{}:</b>" \
                   "\n#UNRESTRICTED" \
                   "\n<b>• Admin:</b> {}" \
...
MUTEME_HANDLER = DisableAbleCommandHandler(["muteme", "mauadmin"], muteme, pass_args=True, filters=Filters.group, admin_ok=True)
