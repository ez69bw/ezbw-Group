...
Btw Paris juga punya bot music, jika kalian ingin silahkan klik [vcg music bot](t.me/anxietyclubbot)

Klik /help atau tombol help untuk melihat cara menggunakan saya dengan kemampuan saya.
"""
...
for module_name in ALL_MODULES:
    ...
    if not imported_module.__mod_name__.lower() in IMPORTED:
        ...
        raise Exception("Tidak bisa mempunyai 2 module dengan nama yang sama!")
...
def test(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text("Dia mengedit pesan")
...
def start(bot: Bot, update: Update, args: List[str]):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text("Saya disini")
...
def send_start(bot, update):
    ...
    text = PM_START.format(escape_markdown(first_name), escape_markdown(bot.first_name))

    keyboard = [[InlineKeyboardButton(text="INVITE GROUP", url="t.me/{}?startgroup=true".format(bot.username))]]
    keyboard = [[InlineKeyboardButton(text="HELP", callback_data="help_back")]]
    keyboard += [[InlineKeyboardButton(text="LANGUAGE", callback_data="set_lang_"),
        InlineKeyboardButton(text="REPORTING", callback_data="cntrl_panel_M")]]
...
def control_panel(bot, update):
    ...
    if query:
        ...
        M_match = "paris gamtenk mwehehe" #LMAO, don't uncomment
    ...
    if M_match:
        ...
        if conn:
            ...
            if member.status in ('administrator', 'creator'):
                text += f"\nChat yang terhubung - *{chatG.title}* (you {member.status})"
                keyboard += [[InlineKeyboardButton(text="👥 Group settings", callback_data="cntrl_panel_G_back")]]
            elif user.id in SUDO_USERS:
                text += f"\nChat yang terhubung - *{chatG.title}* (you sudo)"
                keyboard += [[InlineKeyboardButton(text="👥 Group settings (SUDO)", callback_data="cntrl_panel_G_back")]]
            else:
                text += f"\nConnected chat - *{chatG.title}* (anda siapa nyuruh saya? anda bukan admin!)"
        else:
            text += "\nTidak ada chat yang terhubung!"
        ...
        if mod_match:
            ...
            text = "Anda telah mengikuti pengaturan module *{}*:\n\n".format(
        ...
        if not conn == False:
            ...
            query.message.reply_text(text="Kesalahan dalam koneksi")
            exit(1)
        ...
        if mod_match:
            ...
            text = "*{}* telah mengikuti pengaturan module *{}*:\n\n".format(
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = tld(chat.id, "Ini adalah bantuan module *{}*:\n{}").format(mod_name, help_txt)
...
def send_settings(chat_id, user_id, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id, "Pengaturanmu saat ini:" + "\n\n" + settings,
                                        parse_mode=ParseMode.MARKDOWN)
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id,
                                        text="Module mana yang ingin anda periksa pengaturan {}?".format(
                                            chat_name),
                                        reply_markup=InlineKeyboardMarkup(
                                            paginate_modules(user_id, 0, CHAT_SETTINGS, "stngs", chat=chat_id)))
        else:
            dispatcher.bot.send_message(user_id, "Sepertinya tidak ada pengaturan chat yang tersedia :'(\nKirim ini "
...
def settings_button(bot: Bot, update: Update):
    ...
    try:
        ...
        if excp.message == "Pesan tidak di edit":
            pass
        elif excp.message == "Query_id_invalid":
            pass
